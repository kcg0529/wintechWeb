...
class CycleDAO {
    ...
    public static function getTotalCycles($search_condition = '') {
        $conn = getConnection();
        $count_query = "SELECT COUNT(*) as total FROM wintech_cycle $search_condition";
    ...
    public static function getCycles($search_condition = '', $limit = 10, $offset = 0) {
        $conn = getConnection();
        $query = "SELECT * FROM wintech_cycle $search_condition ORDER BY no DESC LIMIT $limit OFFSET $offset";
     ...
     * 특정 사이클 데이터 조회
     */
    public static function getCycleById($cycle_id) {
        $conn = getConnection();
        $query = "SELECT * FROM wintech_cycle WHERE no = ?";
     ...
     * 사이클 데이터 삭제
     */
    public static function deleteCycle($cycle_id) {
        $conn = getConnection();
        $query = "DELETE FROM wintech_cycle WHERE no = ?";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, "i", $cycle_id);
    ...
    public static function getCycleStats() {
        ...
        // 총 세션 수
        $total_query = "SELECT COUNT(*) as total FROM wintech_cycle";
        $total_result = mysqli_query($conn, $total_query);
        $stats['total_sessions'] = mysqli_fetch_assoc($total_result)['total'];
        
        // 총 사용자 수
        $users_query = "SELECT COUNT(DISTINCT name) as total FROM wintech_cycle";
        $users_result = mysqli_query($conn, $users_query);
        $stats['total_users'] = mysqli_fetch_assoc($users_result)['total'];
        
        // 평균 운동 시간
        $avg_time_query = "SELECT AVG(exercise_time) as avg_time FROM wintech_cycle";
        $avg_time_result = mysqli_query($conn, $avg_time_query);
        $stats['avg_exercise_time'] = mysqli_fetch_assoc($avg_time_result)['avg_time'];
        
        // 총 거리
        $total_distance_query = "SELECT SUM(distance) as total_distance FROM wintech_cycle";
    ...
    public static function tableExists() {
        $conn = getConnection();
        $query = "SHOW TABLES LIKE 'wintech_cycle'";
        $result = mysqli_query($conn, $query);
